...
public class CubicBezierCurve {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector3 GetInverseDirection2() {
        if(n2.UseInverseN2)
        {
            return (2*n2.direction)-n2.direction;
        }
        return n2.direction;
    }
    public Vector3 GetInverseDirection1()
    {
        if (n1.UseInverseN1)
        {
            return (2 * n1.direction) - n1.direction;
        }
        return n1.direction;
    }
    /// <summary>
    ...
    public Vector3 GetLocation(float t)
    {
        ...
        return
            n1.position * (omt2 * omt) +
             GetInverseDirection1() * (3f * omt2 * t) +
            GetInverseDirection2() * (3f * omt * t2) +
            n2.position * (t2 * t);
    }
    ...
    public Vector3 GetTangent(float t)
    {
        ...
        Vector3 tangent =
            n1.position * (-omt2) +
             GetInverseDirection1() * (3 * omt2 - 2 * omt) +
             GetInverseDirection2() * (-3 * t2 + 2 * t) +
            n2.position * (t2);
